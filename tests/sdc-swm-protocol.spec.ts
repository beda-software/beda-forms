...
async function sendRequest(page: Page, messageType: string, payload: Record<string, unknown>) {
...
test("renderer speaks SDC SMART Web Messaging", async ({ page, baseURL }) => {
  ...
  const hostHandshakeMessage = requireMessage(hostHandshakeResponse, "host handshake response");
  ...
  const configureMessage = requireMessage(configureResponse, "sdc.configure response");
  ...
  const contextMessage = requireMessage(contextResponse, "sdc.configureContext response");
  ...
  const displayMessage = requireMessage(displayResponse, "sdc.displayQuestionnaire response");
  ...
  const changeEvent = requireMessage(changeMessage, "sdc.ui.changedQuestionnaireResponse");
  ...
  const currentResponseId = await sendRequest(page, "sdc.requestCurrentQuestionnaireResponse", {});
  ...
  if (currentPayload.questionnaireResponse) {
    expect(currentPayload.questionnaireResponse.resourceType).toBe("QuestionnaireResponse");
  } else {
  ...
  const extractMessage = requireMessage(extractResponse, "sdc.requestExtract response");
